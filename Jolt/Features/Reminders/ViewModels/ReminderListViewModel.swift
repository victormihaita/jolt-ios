...
class ReminderListViewModel: ObservableObject {
    ...
    func deleteReminder(_ id: UUID) async {
        do {
            let mutation = JoltAPI.DeleteReminderMutation(id: id.uuidString)
    ...
    func completeReminder(_ id: UUID) async {
        do {
            let mutation = JoltAPI.CompleteReminderMutation(id: id.uuidString)
            _ = try await graphQL.perform(mutation: mutation)
        } catch {
    ...
    func snoozeReminder(_ id: UUID, minutes: Int) async {
        do {
            let mutation = JoltAPI.SnoozeReminderMutation(id: id.uuidString, minutes: minutes)
            _ = try await graphQL.perform(mutation: mutation)
        } catch {
