...
public final class NotificationService: NSObject, ObservableObject {
    ...
    public func requestAuthorization() async -> Bool {
        do {
            ...
            PRLogger.error("Failed to request notification authorization: \(error)", category: .notifications)
            return false
        }
    }
    ...
    private func registerNotificationCategories() async {
        let snooze5Action = UNNotificationAction(
            identifier: PRConstants.Notifications.actionSnooze5,
            title: "5 min",
            options: []
        )

        let snooze15Action = UNNotificationAction(
            identifier: PRConstants.Notifications.actionSnooze15,
            title: "15 min",
            options: []
        )

        let customSnoozeAction = UNNotificationAction(
            identifier: PRConstants.Notifications.actionSnoozeCustom,
            title: "Custom",
            options: [.foreground]
        )

        let completeAction = UNNotificationAction(
            identifier: PRConstants.Notifications.actionComplete,
            title: "Done",
            options: [.destructive]
        )

        let reminderCategory = UNNotificationCategory(
            identifier: PRConstants.Notifications.categoryReminder,
    ...
    public func scheduleNotification(for reminder: Reminder) async {
        ...
        content.categoryIdentifier = PRConstants.Notifications.categoryReminder
        ...
        do {
            ...
            PRLogger.debug("Scheduled notification for reminder: \(reminder.id)", category: .notifications)
        } catch {
            PRLogger.error("Failed to schedule notification: \(error)", category: .notifications)
        }
    }
...
extension NotificationService: UNUserNotificationCenterDelegate {
    ...
    public func userNotificationCenter(
        ...
        case PRConstants.Notifications.actionSnooze5:
            onSnoozeRequested?(reminderId, 5)
        case PRConstants.Notifications.actionSnooze15:
            onSnoozeRequested?(reminderId, 15)
        case PRConstants.Notifications.actionComplete:
