...
public final class WidgetDataService {
    ...
    private init() {
        userDefaults = UserDefaults(suiteName: JoltConstants.AppGroup.identifier)
        print("🔧 WidgetDataService init - App Group: \(JoltConstants.AppGroup.identifier)")
    ...
    public func saveReminders(_ reminders: [WidgetReminder]) {
        ...
        guard let userDefaults = userDefaults else {
            ...
            JoltLogger.warning("App Group UserDefaults not available", category: .sync)
            return
        }
        ...
        do {
            ...
            JoltLogger.debug("Saved \(reminders.count) reminders to widget storage", category: .sync)
        } catch {
            print("❌ WidgetDataService: Failed to encode - \(error)")
            JoltLogger.error("Failed to encode reminders for widget: \(error)", category: .sync)
        }
    }
    ...
    public func reloadWidgetTimelines() {
        ...
        JoltLogger.debug("Triggered widget timeline reload", category: .sync)
    }
    ...
    public func clearWidgetData() {
        ...
        reloadWidgetTimelines()
        JoltLogger.debug("Cleared widget data", category: .sync)
    }
}
