...
public final class SubscriptionService: NSObject, ObservableObject {
    ...
    public func setUserID(_ userID: String) async {
        do {
            ...
            PRLogger.error("RevenueCat login error: \(error)", category: .subscription)
        }
    }
    ...
    public func logout() async {
        do {
            ...
            PRLogger.error("RevenueCat logout error: \(error)", category: .subscription)
        }
    }
    ...
    public func checkSubscriptionStatus() async {
        do {
            ...
            PRLogger.error("Error fetching customer info: \(error)", category: .subscription)
        }
    }

    @MainActor
    private func updateSubscriptionStatus(_ customerInfo: CustomerInfo) {
        isPremium = customerInfo.entitlements[PRConstants.Premium.entitlementID]?.isActive == true
    }
